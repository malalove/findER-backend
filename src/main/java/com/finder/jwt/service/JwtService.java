...
public class JwtService {
    ...
    public String createAccessToken(String email) {
        ...
        return JWT.create() // JWT 토큰을 생성하는 빌더 반환
                .withSubject(ACCESS_TOKEN_SUBJECT) // Subject 설정
                .withExpiresAt(new Date(now.getTime() + accessTokenExpirationPeriod)) // 토큰 만료 시간 설정
                .withClaim(EMAIL_CLAIM, email) // 토큰에 이메일 정보 저장
                .sign(Algorithm.HMAC512(secretKey)); // HMAC512 암호화 알고리즘 사용
    }
    ...
    // AccessToken과 RefreshToken 응답 헤더 설정
    public void setAccessAndRefreshHeader(HttpServletResponse response, String accessToken, String refreshToken) {
        ...
        log.info("Access Token과 Refresh Token 헤더 설정 완료");
    }
    ...
    public boolean isAccessTokenValid(String token) {
        try {
            ...
            log.error("유효하지 않은 Access Token입니다. {}", e.getMessage());
